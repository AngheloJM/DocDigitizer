...
@pytest.mark.asyncio
async def test_folder_create_endpoint_enforces_rate_limit(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(folders_router, "enforce_rate_limit", _fake_enforce_rate_limit)

    class _FakeUser:
        id = uuid.uuid4()

    with pytest.raises(HTTPException) as exc_info:
        await folders_router.create_folder(data=None, db=None, current_user=_FakeUser())

    assert exc_info.value.status_code == 429
    assert captured["key"] == f"folder_create_rate:{_FakeUser.id}"
